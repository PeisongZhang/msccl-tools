...
def alltoall_ring_ring(num_ranks, instances, gpus_per_row, protocol):
    ...
    collective = AllToAll(num_ranks, 4, inplace=False)
                ...
                for column in range(num_column):
                    ...
                    if step == 0:
                        ...
                        for r in range(num_row):
                            ...
                            chunk_id.append(c_id * 4)
                            chunk_id.append(c_id * 4 + 1)
                        for id in chunk_id:
                    ...
                    if dst_rank1 == dst_rank2: ## dst_column1 == dst_column2
                        src_chunk_id1 = [(r * num_column + dst_column1) * 4 for r in range(num_row)] # forward
                        src_chunk_id2 = [(r * num_column + dst_column2) * 4 + 1 for r in range(num_row)] # backward

                        dst_chunk_id1 = [(r * num_column + src_column) * 4 for r in range(num_row)] # forward
                        dst_chunk_id2 = [(r * num_column + src_column) * 4 + 1 for r in range(num_row)]
                    else:
                        ## forward
                        for r in range(num_row):
                            src_chunk_id1.append((r * num_column + dst_column1) * 4)
                            src_chunk_id1.append((r * num_column + dst_column1) * 4 + 1)

                            dst_chunk_id1.append((r * num_column + src_column) * 4)
                            dst_chunk_id1.append((r * num_column + src_column) * 4 + 1)
                        ## backward
                            src_chunk_id2.append((r * num_column + dst_column2) * 4)
                            src_chunk_id2.append((r * num_column + dst_column2) * 4 + 1)
                            dst_chunk_id2.append((r * num_column + src_column) * 4)
                            dst_chunk_id2.append((r * num_column + src_column) * 4 + 1)
            ...
            for step in range(num_row // 2 + 1):  # 列内步数
                for row in range(num_row): # 列内每一行（每一个rank)
                    ...
                    if dst_rank1 == dst_rank2:
                        src_chunk_id1 = [(dst_row1 * num_column + c) * 4 for c in range(num_column)]
                        src_chunk_id2 = [(dst_row1 * num_column + c) * 4 + 1 for c in range(num_column)]
                        dst_chunk_id1 = [(src_row * num_column + c) * 4 for c in range(num_column)]
                        dst_chunk_id2 = [(src_row * num_column + c) * 4 + 1 for c in range(num_column)]
                    else:
                        for c in range(num_column):
                            src_chunk_id1.append((dst_row1 * num_column + c) * 4)
                            src_chunk_id1.append((dst_row1 * num_column + c) * 4 + 1)
                            dst_chunk_id1.append((src_row * num_column + c) * 4)
                            dst_chunk_id1.append((src_row * num_column + c) * 4 + 1)
                        
                            src_chunk_id2.append((dst_row2 * num_column + c) * 4)
                            src_chunk_id2.append((dst_row2 * num_column + c) * 4 + 1)
                            dst_chunk_id2.append((src_row * num_column + c) * 4)
                            dst_chunk_id2.append((src_row * num_column + c) * 4 + 1)
            ...
            for step in range(num_row // 2 + 1):  # 列内步数
                for row in range(num_row): # 列内每一行（每一个rank)
                    ...
                    if step == 0:
                        ...
                        for c in range(num_column):
                            c_id = num_column * row + c
                            chunk_id.append(c_id * 4 + 2)
                            chunk_id.append(c_id * 4 + 3)
                        for id in chunk_id:
                            chunk(rank, Buffer.input, id).copy(rank, Buffer.scratch, id)
                        continue
                    assert step != 0
                    ...
                    if dst_rank1 == dst_rank2:
                        src_chunk_id1 = [(dst_row1 * num_column + c) * 4 + 2 for c in range(num_column)]
                        src_chunk_id2 = [(dst_row1 * num_column + c) * 4 + 3 for c in range(num_column)]
                        dst_chunk_id1 = [(src_row * num_column + c) * 4 + 2 for c in range(num_column)]
                        dst_chunk_id2 = [(src_row * num_column + c) * 4 + 3 for c in range(num_column)]
                    else:
                        for c in range(num_column):
                            src_chunk_id1.append((dst_row1 * num_column + c) * 4 + 2)
                            src_chunk_id1.append((dst_row1 * num_column + c) * 4 + 3)
                            dst_chunk_id1.append((src_row * num_column + c) * 4 + 2)
                            dst_chunk_id1.append((src_row * num_column + c) * 4 + 3)
                        
                            src_chunk_id2.append((dst_row2 * num_column + c) * 4 + 2)
                            src_chunk_id2.append((dst_row2 * num_column + c) * 4 + 3)
                            dst_chunk_id2.append((src_row * num_column + c) * 4 + 2)
                            dst_chunk_id2.append((src_row * num_column + c) * 4 + 3)
        ...
        for row in range(num_row):
            for step in range(num_column // 2 + 1):
                for column in range(num_column):
                    ...
                    if dst_rank1 == dst_rank2: ## dst_column1 == dst_column2
                        src_chunk_id1 = [(r * num_column + dst_column1) * 4 + 2 for r in range(num_row)] # forward
                        src_chunk_id2 = [(r * num_column + dst_column2) * 4 + 3 for r in range(num_row)] # backward

                        dst_chunk_id1 = [(r * num_column + src_column) * 4 + 2 for r in range(num_row)] # forward
                        dst_chunk_id2 = [(r * num_column + src_column) * 4 + 3 for r in range(num_row)]
                    else:
                        ## forward
                        for r in range(num_row):
                            src_chunk_id1.append((r * num_column + dst_column1) * 4 + 2)
                            src_chunk_id1.append((r * num_column + dst_column1) * 4 + 3)

                            dst_chunk_id1.append((r * num_column + src_column) * 4 + 2)
                            dst_chunk_id1.append((r * num_column + src_column) * 4 + 3)
                        ## backward
                            src_chunk_id2.append((r * num_column + dst_column2) * 4 + 2)
                            src_chunk_id2.append((r * num_column + dst_column2) * 4 + 3)
                            dst_chunk_id2.append((r * num_column + src_column) * 4 + 2)
                            dst_chunk_id2.append((r * num_column + src_column) * 4 + 3)
