...
def alltoall(num_ranks, instances, protocol):
    ...
    with MSCCLProgram("alltoall_allpairs", topology, collective, instances=instances, protocol=protocol, threadblock_policy=ThreadblockPolicy.manual):
        for r in range(num_ranks):
            for index in range(num_ranks):
                c = chunk(r, Buffer.input, index)
                c.copy(index, Buffer.output, r, sendtb=index, recvtb=r + num_ranks, ch=index)
        XML()
        Check()
