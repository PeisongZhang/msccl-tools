...
def alltoall_p2p_p2p(num_ranks, instances, gpus_per_node, protocol):
    topology = fully_connected(num_ranks)
    collective = AllToAll(num_ranks, 1, inplace=False)

    with MSCCLProgram("alltoall_p2p_p2p", topology, collective, instances=instances, \
        ...
        # --- Phase 2: (Inter-node) P2P ---
        for n in range(num_nodes):
            for g in range(gpus_per_node):
                src_rank = g + n * gpus_per_node 
                for dst_n in range(num_nodes):
                    dst_g = g
...
alltoall_p2p_p2p(args.num_gpus, args.instances, args.gpus_per_node, args.protocol)
