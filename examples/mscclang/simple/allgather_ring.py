...
def allgather_ring_impl(size):
    ...
    for r in range(size):
        ...
        while next != r:
            ...
            c = c.copy(next, buffer=Buffer.output, index=r, sendtb=next, recvtb=(size + (next - 1) % size))
            next = (next + 1) % size
...
def allgather_ring(size):
    ...
    with MSCCLProgram("allgather_ring", topology, collective, 1, 'Simple', ThreadblockPolicy.manual):
        allgather_ring_impl(size)
        XML()
        Check()
